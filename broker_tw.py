...
def cook(filelist):
    ...
    for filename in filelist:
        ...
        # set the output path
        output_folder = "./broker_tw/{}/".format(ticker)
        if not os.path.exists(output_folder):
            os.makedirs(output_folder, mode=0o777)
        output_file = "{}_broker_{}.csv".format(ticker, date.strftime("%Y-%m-%d"))
        ...
        if os.stat(filename).st_size == 0:
            print("{} is an empty file, and will be skipped.".format(filename))
            skip += 1
        ...
        ga.insert(1, "ticker", "{}.TW".format(ticker))
